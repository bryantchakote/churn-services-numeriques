...
def plot_disc_vs_target(df, cols, segment="All", target="Churn Value", **kwargs):
    ...
    for i, col in enumerate(cols):
        ...
        bar = prop_df.plot(
            kind="bar", stacked=True, legend=False, color=colors_01, alpha=0.8, ax=ax
        )
...
def plot_cont_vs_cont(df, cols, segment="All", target="Churn Value", **kwargs):
    ...
    g = sns.PairGrid(
        df, hue=target, vars=cols, palette=colors_01, corner=True, diag_sharey=False
    )
    ...
    plt.suptitle(
        f"Relations entre les variables continues, Segment = {segment}",
        y=1.01,
        size=14,
        weight="bold",
    )
...
def plot_cont_vs_target(
    ...
    plt.suptitle(
        f"Distribution des variables continues\nSegment = {segment}, Churn Value = {target_value}",
        y=0.95,
        size=12,
        weight="bold",
    )
