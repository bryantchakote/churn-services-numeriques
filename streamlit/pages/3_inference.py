...
if "preprocessor_internet" not in st.session_state:
	st.session_state["preprocessor_internet"] = joblib.load("artifacts/internet/preprocessor_internet.pkl")

if "model_internet" not in st.session_state:
	st.session_state["model_internet"] = joblib.load("artifacts/internet/model_internet.pkl")

# Artefacts (Sans Internet)
if "preprocessor_no_internet" not in st.session_state:
	st.session_state["preprocessor_no_internet"] = joblib.load("artifacts/no_internet/preprocessor_no_internet.pkl")

if "model_no_internet" not in st.session_state:
	st.session_state["model_no_internet"] = joblib.load("artifacts/no_internet/model_no_internet.pkl")

# Artefacts (All)
if "preprocessor_all" not in st.session_state:
	st.session_state["preprocessor_all"] = joblib.load("artifacts/all/preprocessor_all.pkl")
...
if multiple_predictions:
    uploaded_file = st.file_uploader("Veuillez uploader un fichier CSV pour effectuer un ensemble de prédictions.", type=["csv"])
        ...
        for j, col in enumerate(disc_cols):
            with col:
                ...
                if col_name == "Multiple Lines":
                    col_values = col_values[0] if disc["Phone Service"] == "Yes" else col_values[1]
                
                if col_name in INTERNET_SERVICES:
                    col_values = col_values[0] if disc["Internet Service"] != "No" else col_values[1]
    ...
    df_internet, df_no_internet = prepare_data(input_data, extract_coords=False, identify_new_clients=True, split_internet=True)
    ...
    signal = "🟢" if churn_score < 0.3 else "🟡" if churn_score < 0.5 else "🟠" if churn_score < 0.7 else "🔴"
